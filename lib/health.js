...
import { getKEVAge, getDatabaseHealth } from './db.js';
import { getUserSettingsStatus } from './user-settings.js';
import { getGenerationStatus } from './generation-jobs.js';

// "Reachable" = fetched or revalidated successfully, even if quiet (empty).
// Serving stale cache after a failed request is retained coverage, not proof
// that a publisher is reachable. `fresh` is the narrower "had current items" count.
//
...
function getDatabaseStats(dataDir) {
  try {
    ...
    const mainBytes = statSync(dbPath).size;
    ...
    const sizeMB = Math.round((mainBytes + walBytes + shmBytes) / 1024 / 1024 * 10) / 10;
    const health = getDatabaseHealth();
    const status = health.status === 'error' ? 'error' : sizeMB > 500 ? 'warning' : sizeMB > 100 ? 'growing' : 'ok';
    return { size_mb: sizeMB, main_bytes: mainBytes, wal_bytes: walBytes, shm_bytes: shmBytes, status, readable: health.readable, persistence: health.persistence };
  } catch {
...
// Liveness answers one question only: is the HTTP process running? It must not
// fail because feeds, the database, or AI are temporarily unavailable, or a
// container orchestrator can turn a recoverable dependency outage into a
// restart loop.
...
export function healthHandler({
  ...
  return (req, res) => {
    ...
    const settings = getUserSettingsStatus();
    const generation = getGenerationStatus();
    const degraded = staleRun || feedsOutage || dbBroken || settings.status === 'error' || generation.persistence === 'error';
    ...
    // only gate.
    // A loopback bind is not proof that this request came from the local user:
    // a reverse proxy on the same host also connects over loopback. Whenever
    // API_SECRET is configured, only bearer-authenticated health requests may
    // receive diagnostics, regardless of the bind address.
    const trusted = res.locals.authenticated === true || (loopback && !requireAuthForDetails);
    if (!trusted) {
    ...
    res.status(degraded ? 503 : 200).json({
      ...
      ai: (() => {
        ...
        if (s) return { enabled: s.enabled, source: s.source, rotated: Boolean(s.rotated), ...(s.provider ? { provider: s.provider, model: s.model } : {}) };
        return process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY ? 'configured' : 'disabled';
      })(),
      pipeline: {
      ...
      database: dbStats,
      settings,
      generation: { persistence: generation.persistence, active: generation.active, latest: generation.latest },
      memory: {
...
// Named readiness export for new deployments. Keep healthHandler as the
// implementation and /api/health compatibility contract for existing monitors.
export const readinessHandler = healthHandler;
