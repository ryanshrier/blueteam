...
async function dispatchAlertsOnce(headlines, config) {
  ...
  try {
    ...
    for (const batch of batches) {
      const res = await postWebhook(url, batch.body);
      if (!res.ok) {
        log.warn('alerts', `Webhook POST returned HTTP ${res.status} — leaving undelivered alerts eligible for retry`);
        return;
      }
      delivered.push(...batch.keys);
      // Checkpoint each confirmed batch, never the unsent remainder.
      if (batch.keys.length) persistSentKeys(delivered);
    }
