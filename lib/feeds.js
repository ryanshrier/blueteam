...
const MAX_CACHED_ITEMS_JSON = 128_000;
...
function mergeTextEvidence(current, candidate) {
  const a = typeof current === 'string' ? current : '';
  const b = typeof candidate === 'string' ? candidate : '';
  if (b.length !== a.length) return b.length > a.length ? b : a;
  return [a, b].sort((x, y) => x.localeCompare(y))[0];
}
...
export function deduplicateWithCorroboration(headlines, threshold = 0.55) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (dupIdx >= 0) {
      ...
      survivor.description = mergeTextEvidence(survivor.description, h.description);
      if (survivor.articleBody || h.articleBody) {
        survivor.articleBody = mergeTextEvidence(survivor.articleBody, h.articleBody);
      }

      // Published time is factual evidence rather than a privilege of whichever
      // feed happened to finish first. Keep the earliest valid report time.
      const survivorTime = validTimestamp(survivor.date);
      const candidateTime = validTimestamp(h.date);
      if (candidateTime !== null && (survivorTime === null || candidateTime < survivorTime)) {
        survivor.date = h.date;
        survivor.dateUnknown = false;
      } else if (survivorTime !== null) {
        survivor.dateUnknown = false;
      }
    } else {
      kept.push(h);
...
function normalizeCachedItem(item, feed) {
...
function parseCachedItems(cached, feed) {
  ...
  return parsed
    .slice(0, 5)
    .map(item => normalizeCachedItem(item, feed))
    .filter(Boolean);
}
