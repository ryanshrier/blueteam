...
function originalSourceMembers(headline) {
  ...
  const fields = ['title', 'source', 'link', 'description', 'passage', 'passageTruncated', 'feedUrl', 'sourceIdentifier', 'date', 'dateUnknown', 'retrievedAt', 'collectionStale', 'sourceUpdatedAt', 'category', 'evidence'];
...
export function deduplicateWithCorroboration(headlines, threshold = 0.55) {
  ...
  const candidates = headlines.map((headline) => {
    ...
    copy.publishers = [...new Set([
      ...(Array.isArray(headline.publishers) ? headline.publishers : []),
      publisherKey(headline),
    ].filter(value => typeof value === 'string' && value))].sort((a, b) => a.localeCompare(b));
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    for (const j of [...similarities.keys()].sort((a, b) => a - b)) {
      if (similarities.get(j) + Number.EPSILON >= threshold) {
...
function normalizeCachedItem(item, feed, cachedAt) {
  ...
  const date = textOf(item.date, FEED_FIELD_LIMITS.dateRaw);
  return {
    title,
...
function parseSearchItems(xml, horizon) {
  ...
  for (const item of items) {
    ...
    if (!title) continue;
    const date = textOf(item.pubDate || '', FEED_FIELD_LIMITS.dateRaw);
