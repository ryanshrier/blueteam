...
import { normalizeFeedTimestamp, sameVulnerabilityDevelopment, mustKeepSeparateDevelopments } from './intelligence-context.js';
...
import {
  getFeedCache, setFeedCache, logFeedHealth,
  getExternalCache, setExternalCache,
} from './db.js';
...
import { observeSources } from './evidence.js';
import { getEffectiveWatchProfile } from './user-settings.js';
import { snapshotScoringConfiguration } from './scoring-snapshot.js';
...
function feedBodyCap(feedUrl) {
  try {
    ...
    const hit = FEED_BODY_CAP_OVERRIDES.find(([needle]) =>
...
// A feed body may legitimately be several MB, but no individual headline field
// needs to be. Bound attacker-controlled strings BEFORE entity decoding, HTML
// stripping, regex matching, URL parsing, or date parsing so one oversized XML
// field cannot multiply memory use in a synchronous transform.
export const FEED_FIELD_LIMITS = Object.freeze({
  titleRaw: 4_096,
  title: 512,
  descriptionRaw: 8_192,
  description: 300,
  linkRaw: 4_096,
  dateRaw: 256,
  sourceRaw: 1_024,
...
const MAX_ITEMS_SCANNED_PER_FEED = 200;
const MAX_CACHED_ITEMS_JSON = 4_000_000;
...
export function stripHtml(str, rawLimit = FEED_FIELD_LIMITS.descriptionRaw) {
  if (!str || typeof str !== 'string') return '';
  const bounded = str.slice(0, rawLimit);
  return decodeEntities(decodeEntities(bounded))
    .replace(/<[^>]+>/g, ' ')
    .replace(/\s+/g, ' ')
    .replace(/\s+([.,;:!?])/g, '$1')
    .trim();
...
function textOf(node, maxLength) {
  if (!node) return '';
  let value = node;
  if (typeof node === 'object' && node['#text'] !== undefined) value = node['#text'];
  // Slice before trim: String#trim scans the complete input, which is exactly
  // what an oversized feed field should not be able to force.
...
function normalizedFeedText(node, rawLimit, finalLimit) {
  return truncateText(stripHtml(textOf(node, rawLimit), rawLimit), finalLimit);
}

function normalizedSource(node) {
  return normalizedFeedText(
    node,
    FEED_FIELD_LIMITS.sourceRaw,
    FEED_FIELD_LIMITS.source,
  );
}

function normalizedCategory(node) {
  return normalizedFeedText(
    node,
    FEED_FIELD_LIMITS.categoryRaw,
    FEED_FIELD_LIMITS.category,
  );
}
...
function resolveLink(rawLink, baseUrl) {
  if (!rawLink) return '';
  try {
    const bounded = textOf(rawLink, FEED_FIELD_LIMITS.linkRaw);
    if (!bounded) return '';
    const resolved = baseUrl ? new URL(bounded, baseUrl) : new URL(bounded);
    if (!/^https?:$/.test(resolved.protocol)) return '';
    if (resolved.username || resolved.password) return '';
    return resolved.href.length <= FEED_FIELD_LIMITS.linkRaw ? resolved.href : '';
  } catch {
    return '';
  }
}
...
function extractLink(item, baseUrl) {
  ...
  if (item.link && typeof item.link === 'string') {
    raw = textOf(item.link, FEED_FIELD_LIMITS.linkRaw);
  }
  else if (item.link && typeof item.link === 'object') {
    if (item.link['@_href']) {
      raw = textOf(item.link['@_href'], FEED_FIELD_LIMITS.linkRaw);
    }
    else if (Array.isArray(item.link)) {
      const alt = item.link.find(l => l['@_rel'] === 'alternate' || !l['@_rel']);
      if (alt?.['@_href']) raw = textOf(alt['@_href'], FEED_FIELD_LIMITS.linkRaw);
    }
  }
  if (!raw) {
    const guid = textOf(item.guid, FEED_FIELD_LIMITS.linkRaw);
    if (guid.startsWith('http')) raw = guid;
  }
...
function decodeGoogleArticleId(id) {
  try {
    const b64 = textOf(id, FEED_FIELD_LIMITS.linkRaw).replace(/-/g, '+').replace(/_/g, '/');
    ...
    const resolved = resolveLink(candidate);
    if (!resolved || !new URL(resolved).hostname.includes('.')) return '';
    return resolved;
  } catch {
    return '';
  }
...
export function resolveGoogleNewsLink(link, rawDescription = '') {
  const boundedLink = resolveLink(link);
  if (!boundedLink) return '';
  if (!isGoogleNewsLink(boundedLink)) return boundedLink;
  try {
    // 1. description anchor (older feeds embed the real article link here)
    if (rawDescription) {
      const description = textOf(rawDescription, FEED_FIELD_LIMITS.descriptionRaw);
      const hrefs = description.match(/href="(https?:\/\/[^"]+)"/gi) || [];
      for (const h of hrefs) {
        const resolved = resolveLink(h.slice(6, -1));
        if (resolved && !isGoogleNewsLink(resolved)) return resolved;
      }
    }
    // 2. decode the article id from the path
    const m = boundedLink.match(/\/rss\/articles\/([^?/]+)/) || boundedLink.match(/\/articles\/([^?/]+)/);
    if (m) {
...
export function isFresh(dateStr, maxAgeMs) {
  if (!dateStr) return true;
  const parsed = Date.parse(normalizeFeedTimestamp(dateStr));
...
const PUBLISHER_DOMAIN_ALIASES = new Map([['theregister.co.uk', 'theregister.com']]);
const canonicalPublisherDomain = value => PUBLISHER_DOMAIN_ALIASES.get(value) || value;

// A search result's source label can identify the same publisher as a resolved
// feed URL. Learn only unambiguous label/domain pairs from this collection. This
// does not treat a publisher count as independent observation or resolve a URL.
function publisherResolver(headlines) {
  const aliases = new Map();
  for (const member of headlines.flatMap(originalSourceMembers)) {
    const name = normalizeSourceName(member.source);
    const domain = canonicalPublisherDomain(registrableDomain(member.link));
    if (!domain) continue;
    // Discovery sometimes labels a publisher with its literal hostname instead
    // of its display name. Link those spellings only to a domain actually seen
    // in this collection; do not manufacture an article URL from a source label.
    for (const alias of [name, normalizeSourceName(domain), normalizeSourceName(`www.${domain}`)].filter(Boolean)) {
      if (!aliases.has(alias)) aliases.set(alias, new Set());
      aliases.get(alias).add(domain);
    }
  }
  return member => {
    const domain = canonicalPublisherDomain(registrableDomain(member.link));
    if (domain) return domain;
    const matching = aliases.get(normalizeSourceName(member.source));
    return matching?.size === 1 ? [...matching][0] : publisherKey(member);
  };
}
...
export function canonicalArticleUrl(value) {
  try {
    ...
    if (!/^https?:$/.test(url.protocol) || url.username || url.password || isGoogleHost(url.hostname)) return null;
    url.hash = '';
...
function articleTitle(headline) {
  const source = String(headline.source || '').trim();
  const title = String(headline.title || '').normalize('NFKC').replace(/\s+/g, ' ').trim();
  const suffix = title.match(/\s+[-–—|]\s+([^|]+)$/)?.[1];
  return (suffix && normalizeSourceName(suffix) && normalizeSourceName(suffix) === normalizeSourceName(source)
    ? title.slice(0, title.length - suffix.length).replace(/\s+[-–—|]\s*$/, '') : title).toLowerCase();
}

function articleResolver(headlines, resolvePublisher) {
  const urlsByTitle = new Map();
  const titleKey = h => `${resolvePublisher(h)}\u0000${articleTitle(h)}`;
  for (const member of headlines.flatMap(originalSourceMembers)) {
    const url = canonicalArticleUrl(member.link);
    if (!url) continue;
  ...
  return member => {
    const direct = canonicalArticleUrl(member.link);
    const matches = urlsByTitle.get(titleKey(member));
    const resolved = direct || (matches?.size === 1 ? [...matches][0] : null);
    return { articleId: createHash('sha256').update(resolved || titleKey(member)).digest('hex'),
      publisherId: resolvePublisher(member), canonicalUrl: resolved,
      basis: direct ? 'publisher-url' : resolved ? 'same-publisher-exact-title' : 'publisher-and-title',
      evidenceRefs: (member.evidence || []).map(ref => ({ sourceId: ref.sourceId, revisionId: ref.revisionId })) };
  };
}

// ── Deterministic clustering and primary-source selection ──
// Feed completion order is not an editorial signal. Normalize and sort candidates
// before the greedy pass so every input permutation produces the same clusters.
// The sort deliberately puts weighted, first-party/advisory reporting first; the
// winning primary supplies title/source/link while corroborating rows contribute
// provenance and the strongest available description/date evidence. We keep the
// conservative no-chaining behavior of the old pass: a bridge joins one stable
// primary rather than unioning unrelated end points through transitivity.

const AUTHORITY_CATEGORY_RANK = new Map([
  ['gov-advisory', 6],
  ['ics-advisory', 6],
  ['vendor-advisory', 5],
  ['standards', 5],
...
function authorityRank(headline) {
  const source = String(headline?.source || '').toLowerCase();
  const sourceBonus = /\b(?:cisa|nist|cert|jpcert|ncsc|cccs)\b/.test(source) ? 1 : 0;
  return (AUTHORITY_CATEGORY_RANK.get(String(headline?.category || '').toLowerCase()) || 0) + sourceBonus;
}

function stableHeadlineKey(headline) {
  return [
    publisherKey(headline),
    String(headline?.source || '').toLowerCase(),
    String(headline?.title || '').toLowerCase(),
...
function comparePrimaryCandidates(a, b) {
  ...
  const authority = authorityRank(b) - authorityRank(a);
  if (authority) return authority;
  const traceability = Number(Boolean(b?.link)) - Number(Boolean(a?.link));
  if (traceability) return traceability;
  return stableHeadlineKey(a).localeCompare(stableHeadlineKey(b));
}

function originalSourceMembers(headline) {
  if (Array.isArray(headline.sourceMembers)) return headline.sourceMembers.map(member => ({ ...member }));
  const fields = ['title', 'source', 'link', 'description', 'passage', 'passageTruncated', 'feedUrl', 'sourceIdentifier', 'date', 'originalDate', 'dateUnknown', 'retrievedAt', 'collectionStale', 'sourceUpdatedAt', 'category', 'evidence'];
  return [Object.fromEntries(fields.filter(field => headline[field] !== undefined).map(field => [field, headline[field]]))];
}
...
export function deduplicateWithCorroboration(headlines, threshold = 0.55) {
  ...
  const resolvePublisher = publisherResolver(headlines);
  const resolveArticle = articleResolver(headlines, resolvePublisher);

  const candidates = headlines.map((headline) => {
    const copy = { ...headline };
    copy.sourceMembers = originalSourceMembers(headline);
    copy.evidence = Array.isArray(headline.evidence) ? [...headline.evidence] : [];
    copy.sources = [...new Set([
      ...(Array.isArray(headline.sources) ? headline.sources : []),
      ...(headline.source ? [headline.source] : []),
    ].filter(value => typeof value === 'string' && value))].sort((a, b) => a.localeCompare(b));
    // Recalculate from original members: old grouped counts may themselves
    // contain a domain/name duplicate. Never preserve that inflation on replay.
    copy.publishers = [...new Set(copy.sourceMembers.map(resolvePublisher))].sort((a, b) => a.localeCompare(b));
    copy.articleIdentities = copy.sourceMembers.map(resolveArticle);
    copy.corroboration = copy.publishers.length;
    return copy;
  }).sort(comparePrimaryCandidates);

  const allTokens = candidates.map(h => tokenize(h.title));
  const idf = buildIDF(allTokens);

  const kept = [];
  // Sparse normalized vectors avoid a full allocation/normalization for every
  // pair. Only survivors sharing a title term can reach positive similarity.
  const postings = new Map();
  const cvePostings = new Map();
  const keptCves = [];
  const eventPostings = new Map();
  const articlePostings = new Map();
  const addPosting = (index, term, candidate, value) => {
    if (!index.has(term)) index.set(term, []);
    index.get(term).push([candidate, value]);
  };

  for (let i = 0; i < candidates.length; i++) {
    const h = candidates[i];
    // Advisory titles can be templates without an identifier (CISA's "Adds
    // One/Seven/Two ... to Catalog"). Their original passage identifies the
    // actual event. Prefer explicit title IDs, then the unmerged publisher
    // passage; never borrow identifiers from enrichment or grouped members.
    const titleCves = extractCveIds(h.title);
    const cves = titleCves.size ? titleCves : extractCveIds(h.passage || h.description || '');
    const vec = tfidfVector(allTokens[i], idf);
    const magnitude = Math.sqrt(Object.values(vec).reduce((sum, value) => sum + value * value, 0));
    for (const term of Object.keys(vec)) vec[term] /= magnitude || 1;
    let dupIdx = -1;
    const similarities = new Map();
    // An identified event can only match an unidentified or shared-CVE event.
    // Partition these postings before computing similarities: burst feeds with
    // thousands of differently numbered advisories must not force all pairs.
    const indexes = cves.size
      ? ['', ...cves].map(cve => cvePostings.get(cve)).filter(Boolean)
      : [postings];
    for (const [term, value] of Object.entries(vec)) {
      const seen = new Set();
      for (const index of indexes) {
        for (const [j, otherValue] of index.get(term) || []) {
          if (seen.has(j)) continue;
          seen.add(j);
          similarities.set(j, (similarities.get(j) || 0) + value * otherValue);
        }
      }
    }
    const eventCandidates = cves.size === 1 ? (eventPostings.get([...cves][0]) || []).filter(index => sameVulnerabilityDevelopment(h, kept[index])) : [];
    const sameArticles = h.articleIdentities.flatMap(article => articlePostings.get(article.articleId) || []);
    for (const j of [...new Set([...similarities.keys(), ...eventCandidates, ...sameArticles])].sort((a, b) => a - b)) {
      if (sameArticles.includes(j) || eventCandidates.includes(j) || similarities.get(j) + Number.EPSILON >= threshold) {
        // Templated advisory titles can clear the cosine threshold while
        // their original passages identify distinct CVEs. Do not hide the
        // second event inside an unrelated surviving headline.
        if (cves.size && keptCves[j].size && ![...cves].some(cve => keptCves[j].has(cve))) continue;
        if (mustKeepSeparateDevelopments({ ...h, title: articleTitle(h) }, { ...kept[j], title: articleTitle(kept[j]) })) continue;
        dupIdx = j;
        break;
      }
    }
    ...
    if (dupIdx >= 0) {
      ...
      survivor.publishers = [...new Set([...survivor.publishers, ...h.publishers])]
        .sort((a, b) => a.localeCompare(b));
      survivor.corroboration = survivor.publishers.length;
      survivor.sources = [...new Set([...survivor.sources, ...h.sources])]
        .sort((a, b) => a.localeCompare(b));
      survivor.sourceMembers.push(...h.sourceMembers);
      survivor.articleIdentities.push(...h.articleIdentities);
      survivor.evidence = [...new Map([...survivor.evidence, ...h.evidence]
        .map(ref => [`${ref.sourceId}:${ref.revisionId}`, ref])).values()];
      // Title, passage, URL and publication date are one publisher's record.
      // Other reporting stays in sourceMembers with its own attribution.
    } else {
      const index = kept.length;
      kept.push(h);
      for (const article of h.articleIdentities) {
        if (!articlePostings.has(article.articleId)) articlePostings.set(article.articleId, []);
        articlePostings.get(article.articleId).push(index);
      }
      keptCves.push(cves);
      if (cves.size === 1) {
        const id = [...cves][0];
        if (!eventPostings.has(id)) eventPostings.set(id, []);
        eventPostings.get(id).push(index);
      }
      for (const cve of cves.size ? cves : ['']) {
        if (!cvePostings.has(cve)) cvePostings.set(cve, new Map());
      }
      for (const [term, value] of Object.entries(vec)) {
        addPosting(postings, term, index, value);
        for (const cve of cves.size ? cves : ['']) addPosting(cvePostings.get(cve), term, index, value);
      }
    }
  }
  for (const headline of kept) {
    headline.eventIdentity = {
      eventId: createHash('sha256').update(resolveArticle(headline).articleId).digest('hex'),
      basis: headline.articleIdentities.length > 1 ? 'conservative-report-group' : 'single-report',
      publisherCount: headline.publishers.length,
      articleCount: new Set(headline.articleIdentities.map(article => article.articleId)).size,
      retainedRecordCount: headline.sourceMembers.length,
      independentObservation: 'unassessed',
    };
  }
  return kept;
}
...
let collectionLimits = { scanLimitedSources: [], itemsNotScanned: 0 };
...
function normalizeCachedItem(item, feed, cachedAt) {
  if (!item || typeof item !== 'object' || Array.isArray(item)) return null;
  const title = normalizedFeedText(
    item.title,
    FEED_FIELD_LIMITS.titleRaw,
    FEED_FIELD_LIMITS.title,
  );
  if (!title) return null;

  const originalDate = textOf(item.originalDate || item.date, FEED_FIELD_LIMITS.dateRaw);
  const date = normalizeFeedTimestamp(originalDate) || originalDate;
  return {
    title,
    description: normalizedFeedText(
      item.description,
      FEED_FIELD_LIMITS.descriptionRaw,
      FEED_FIELD_LIMITS.description,
    ),
    passage: typeof item.passage === 'string' ? item.passage.slice(0, FEED_FIELD_LIMITS.descriptionRaw) : undefined,
    passageTruncated: Boolean(item.passageTruncated),
    feedUrl: resolveLink(feed.url),
    sourceIdentifier: textOf(item.sourceIdentifier, 1024),
    sourceUpdatedAt: textOf(item.sourceUpdatedAt, FEED_FIELD_LIMITS.dateRaw),
    retrievedAt: textOf(item.retrievedAt, FEED_FIELD_LIMITS.dateRaw) || cachedAt,
    link: resolveLink(item.link),
    // Routing and labels come from the current, validated feed config. This
    // also prevents an old cache row from reintroducing pre-limit strings.
    source: normalizedSource(feed.source) || 'Unknown source',
    category: normalizedCategory(feed.category || 'general') || 'general',
    horizon: feed.horizon || 2,
    weight: feed.weight ?? 1.0,
    deepExtract: feed.deepExtract === true,
    date, originalDate,
    dateUnknown: !normalizeFeedTimestamp(originalDate),
    corroboration: 1,
  };
}

function parseCachedItems(cached, feed) {
  if (typeof cached?.items_json !== 'string') {
    throw new Error('cached feed items are missing');
  }
  if (cached.items_json.length > MAX_CACHED_ITEMS_JSON) {
    throw new Error('cached feed items exceed the size limit');
  }
  const parsed = JSON.parse(cached.items_json);
  if (!Array.isArray(parsed)) throw new Error('cached feed items are not an array');
  return parsed
    .slice(0, MAX_ITEMS_SCANNED_PER_FEED)
    .map(item => normalizeCachedItem(item, feed, cached.cached_at ? `${cached.cached_at.replace(' ', 'T')}Z` : null))
    .filter(Boolean);
}
...
function serveStale(feed, cached, results, health) {
  ...
  try {
    const items = parseCachedItems(cached, feed);
    if (items.length === 0) return false;
    results.push(...items.map(item => ({ ...item, collectionStale: true })));
...
export async function fetchNewsContext(feeds, config = {}) {
  const results = [];
  const limits = { scanLimitedSources: [], itemsNotScanned: 0 };
  ...
  await pooledMap(feeds, async (feed) => {
    ...
    if (cbIsOpen(feed.source)) {
      if (!serveStale(feed, cached, results, health)) health[feed.source] = FEED_STATUS.CIRCUIT_OPEN;
    ...
    try {
      ...
      if (res.status === 304 && cached?.items_json) {
        ...
        clearTimeout(t);
        try {
          const reloaded = parseCachedItems(cached, feed);
          ...
          for (const item of fresh) item.retrievedAt = new Date().toISOString();
          ...
          if (fresh.length > 0) {
            try { setFeedCache(feed.url, cached.etag || '', cached.last_modified || '', fresh); } catch { /* non-critical */ }
          }
      ...
      if (items.length > MAX_ITEMS_SCANNED_PER_FEED) {
        limits.scanLimitedSources.push(feed.source);
        limits.itemsNotScanned += items.length - MAX_ITEMS_SCANNED_PER_FEED;
      }
      ...
      for (const item of items) {
        if (++scanned > MAX_ITEMS_SCANNED_PER_FEED) break;
        const title = normalizedFeedText(
          item.title,
          FEED_FIELD_LIMITS.titleRaw,
          FEED_FIELD_LIMITS.title,
        ...
        const desc = normalizedFeedText(
          item.description || item.summary || item['content:encoded'] || item.content || '',
          FEED_FIELD_LIMITS.descriptionRaw,
          FEED_FIELD_LIMITS.description,
        );
        const rawPassage = textOf(item.description || item.summary || item['content:encoded'] || item.content || '', FEED_FIELD_LIMITS.descriptionRaw + 1);
        const passage = stripHtml(rawPassage, FEED_FIELD_LIMITS.descriptionRaw);
        ...
        const originalDate = textOf(
          item.pubDate || item.published || item.updated || '',
          FEED_FIELD_LIMITS.dateRaw,
        );
        const date = normalizeFeedTimestamp(originalDate) || originalDate;
        ...
        const dateUnknown = !normalizeFeedTimestamp(originalDate);

        const rec = {
          title,
          description: desc,
          passage,
          passageTruncated: rawPassage.length > FEED_FIELD_LIMITS.descriptionRaw,
          feedUrl: resolveLink(feed.url),
          sourceIdentifier: textOf(item.guid || item.id, 1024),
          sourceUpdatedAt: textOf(item.updated || '', FEED_FIELD_LIMITS.dateRaw),
...
function buildSearchQueries(config) {
  ...
  const topics = getEffectiveWatchProfile(config).intelligenceQuestions || [];
  for (const topic of topics.slice(0, 3)) {
    const boundedTopic = textOf(topic, 256);
    if (boundedTopic) queries.push({ q: `${boundedTopic} security`, horizon: 2 });
  }
  return queries;
}

const SEARCH_CACHE_TTL_MS = 60 * 60 * 1000;
const SEARCH_STALE_MAX_MS = 24 * 60 * 60 * 1000;

function searchCacheKey(query, horizon) {
  return `search:${createHash('sha256').update(`${horizon}\u0000${query}`).digest('hex')}`;
}

function readSearchCache(key) {
  try { return getExternalCache(key, { allowExpired: true }); } catch { return null; }
}

function writeSearchCache(key, items, options = {}) {
  try {
    setExternalCache(key, JSON.stringify(items), { ttlMs: SEARCH_CACHE_TTL_MS, ...options });
  } catch { /* database may not be ready in an embedder */ }
}

function cachedSearchItems(cached) {
  try {
    const value = JSON.parse(cached?.body || '');
    return Array.isArray(value) ? value.map(item => ({ ...item, retrievedAt: item.retrievedAt || cached.fetched_at || null })) : null;
  } catch {
    return null;
  }
}

function parseSearchItems(xml, horizon) {
  ...
  for (const item of items) {
    ...
    if (!title) continue;
    const originalDate = textOf(item.pubDate || '', FEED_FIELD_LIMITS.dateRaw);
    const date = normalizeFeedTimestamp(originalDate) || originalDate;
    const link = resolveGoogleNewsLink(
      extractLink(item),
      textOf(item.description || item.summary || '', FEED_FIELD_LIMITS.descriptionRaw),
    );
    results.push({
      title,
      description: '',
      ...
      retrievedAt: new Date().toISOString(),
      link,
      source: normalizedSource(item.source?.['#text'] || item.source || 'News Search') || 'News Search',
      category: 'search',
      horizon,
      weight: 0.7,
      deepExtract: false,
...
function usableStaleSearch(cached, items) {
  const fetchedAt = Date.parse(cached?.fetched_at || '');
  return items
    && !Number.isNaN(fetchedAt)
    && (Date.now() - fetchedAt) <= SEARCH_STALE_MAX_MS;
}

async function fetchSearchBatch(queries, stats) {
  const results = [];
  for (const { q, horizon } of queries) {
    const cacheKey = searchCacheKey(q, horizon);
    const cached = readSearchCache(cacheKey);
    const cachedItems = cachedSearchItems(cached);
    if (cached && !cached.expired && cachedItems) {
    ...
    try {
      ...
      try {
        ...
        if (cached?.etag) headers['If-None-Match'] = cached.etag;
        if (cached?.last_modified) headers['If-Modified-Since'] = cached.last_modified;
        res = await safeFetch(url, { signal: ctrl.signal, headers });
        if (res.status === 304 && cachedItems) {
          await discardResponseBody(res);
          for (const item of cachedItems) item.retrievedAt = new Date().toISOString();
          writeSearchCache(cacheKey, cachedItems, {
            etag: cached.etag,
      ...
      const items = parseSearchItems(xml, horizon);
      if (items.length === 0) {
        stats.failures++;
        continue;
      }
      ...
      stats.failures++;
      if (usableStaleSearch(cached, cachedItems)) {
        results.push(...cachedItems.map(item => ({ ...item, collectionStale: true })));
        log.warn('search', `Discovery query ${cacheKey.slice(-8)} failed; serving cached results: ${err.message}`);
      } else {
        log.warn('search', `Discovery query ${cacheKey.slice(-8)} failed: ${err.message}`);
      }
    }
  }
  return results;
...
/** Exclude a known off-topic recurring feature, never the security topic "Squid". */
export function isExcludedRecurringFeature(headline, domainId = getDomainPack().id) {
  if (domainId !== 'cyber' || !/^Friday Squid Blogging\s*:/i.test(headline.title || '')) return false;
  try {
    return ['schneier.com', 'www.schneier.com'].includes(new URL(headline.link).hostname.toLowerCase());
  } catch { return false; }
}
...
export async function runIntelligencePipeline(config) {
  ...
  const scoringConfiguration = snapshotScoringConfiguration(config);
  ...
  // 2. Retain each original feed observation BEFORE grouping changes the display
  // description/date. Storage failure leaves Wire useful with no invented refs.
  const collected = [...rssHeadlines, ...searchHeadlines];
  let evidenceError = null;
  try { observeSources(collected); recordStorageOutcome('evidence'); } catch (err) {
    recordStorageOutcome('evidence', err);
    evidenceError = 'Evidence retention unavailable';
    log.warn('evidence', `Could not retain source observations: ${err.message}`);
  }
  // Keep observations inspectable, but do not spend selected Wire/Briefing
  // slots on the feed's recurring food/open-discussion feature.
  const admitted = collected.filter(headline => !isExcludedRecurringFeature(headline));
  const deduped = deduplicateWithCorroboration(admitted, 0.5);
  ...
  applyAlertRules(deduped, getEffectiveAlertRules(config, watchProfile));
  ...
  for (const h of deduped) scoreHeadline(h, config, watchProfile);
  ...
  for (const h of diverse) scoreHeadline(h, config, watchProfile);
  ...
  return {
    ...
    stats: {
      ...
      evidenceError,
      collection: {
        configuredSources: feeds.length,
        freshSources: Object.values(feedHealth).filter(status => FRESH_FEED_STATUSES.includes(status) || status === FEED_STATUS.EMPTY).length,
        staleSources: Object.values(feedHealth).filter(status => status === FEED_STATUS.OK_STALE).length,
        admittedItems: admitted.length,
        editoriallyExcludedItems: collected.length - admitted.length,
        scanLimit: MAX_ITEMS_SCANNED_PER_FEED,
        ...collectionLimits,
      },
    },
  };
