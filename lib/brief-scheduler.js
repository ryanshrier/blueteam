...
function parseState(raw, legacyLastSuccessDate = null) {
  ...
  return {
    ...
    nextEditionDate: typeof source.nextEditionDate === 'string' ? source.nextEditionDate : null,
    nextTimezone: typeof source.nextTimezone === 'string' ? source.nextTimezone : null,
...
export function startDailyBriefSchedule({
  ...
  const armAt = (instant, patch = {}, editionDate = null, timezone = config.timezone) => {
    if (!isCurrent() || !instant) return null;
    const delay = Math.max(0, instant.getTime() - now().getTime());
    persist({ ...patch, nextAttemptAt: instant.toISOString(), nextEditionDate: editionDate || zonedParts(instant, timezone).date, nextTimezone: timezone });
    timer = setTimeoutFn(run, delay);
    return delay;
  };

  const armAfter = (delayMs, patch = {}, editionDate = null, timezone = config.timezone) => {
    const instant = new Date(now().getTime() + Math.max(0, delayMs));
    return armAt(instant, patch, editionDate, timezone);
  };

  const armRetry = (delayMs, patch = {}, editionDate = state.scheduleDate, timezone = state.nextTimezone || config.timezone) => {
    const current = now();
    const retryAt = new Date(current.getTime() + Math.max(0, delayMs));
    const nextDate = tomorrowDateKey(editionDate);
    const nextSlot = scheduleInstant(nextDate, config);
    // A delayed old edition must not consume the following edition's slot.
    // Retire retries at that slot and arm a new, separately identified job.
    if (nextSlot && retryAt >= nextSlot) {
      const today = zonedParts(current, config.timezone).date;
      const date = today > nextDate ? today : nextDate;
      const target = scheduleInstant(date, config);
      return armAt(target && target > current ? target : current, {
        ...patch, outcome: 'scheduled', lastError: `Retries for ${editionDate} retired at the next daily edition.`,
      }, date);
    }
    return armAt(retryAt, patch, editionDate, timezone);
  };
  ...
  const scheduleNext = () => {
    ...
    if (state.scheduleDate && state.lastSuccessDate !== state.scheduleDate && state.attempts < config.maxAttempts && resumableOutcome) {
      const retryAt = Date.parse(state.nextAttemptAt || '');
      const delay = Number.isFinite(retryAt) && retryAt > current.getTime() ? retryAt - current.getTime() : catchupMs;
      // Resume the persisted edition even after midnight and before today's
      // normal slot; it is not a fresh missed run governed by the skip policy.
      return armRetry(delay, { outcome: 'retry-pending' });
    }
    ...
    if (config.missedRun === 'catch-up') {
      return armAfter(catchupMs, { outcome: 'catch-up-pending' }, today);
    }
  ...
  function run() {
    ...
    work.then(() => scheduledWork.delete(work), () => scheduledWork.delete(work));
    return work;
  }
  ...
  async function runAttempt() {
    ...
    if (runningEpoch !== null) {
      armRetry(config.retryMinutes * 60_000, { outcome: 'waiting-for-active-generation' }, state.nextEditionDate || zonedParts(now(), config.timezone).date);
      return;
    }

    const current = now();
    const today = state.nextEditionDate || zonedParts(current, config.timezone).date;
    ...
    const scheduledJob = {
      jobKey,
      editionDate: today,
      timezone: state.nextTimezone || config.timezone,
    };
    ...
    try {
      ...
      armRetry(config.retryMinutes * 60_000, {
        scheduleDate: today,
        attempts,
        jobKey,
...
/** Include the result/legacy ledger writes after the internal HTTP request. */
export async function waitForDailyBriefIdle() {
  while (scheduledWork.size) await Promise.allSettled([...scheduledWork]);
}
