...
// Express's default router matches paths without regard to case. Every guard
// must use that same policy, including when mounted inside the API router.
function isApiRequest(req) {
  const path = String(req.originalUrl || req.url || req.path || '').split('?', 1)[0];
...
export function originCheck({
  ...
  return (req, res, next) => {
    if (!STATE_CHANGING_METHODS.has(req.method) || !isApiRequest(req)) return next();
  ...
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        objectSrc: ["'none'"],
        frameAncestors: ["'none'"],
        // The supported localhost HTTP workflow must work in Safari too;
        // upgrading its assets to HTTPS targets a listener that does not exist.
        upgradeInsecureRequests: req.protocol === 'http' && isLoopbackHost(hostHeaderName(req.headers.host))
          ? null : [],
      },
    },
...
export function createRateLimiters() {
  ...
  // /settings/verify makes a real provider call like generation does, but sits
...
export function contentTypeCheck(req, res, next) {
  if (STATE_CHANGING_METHODS.has(req.method) && isApiRequest(req)) {
