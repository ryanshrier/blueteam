...
import { normalizeFeedTimestamp, headlineCves, assessUrgency, currentEventStatements } from './intelligence-context.js';
...
function recencyUnit(headline, halfLifeHours) {
  const t = Date.parse(normalizeFeedTimestamp(headline.date));
 ...
 */
export function scoreHeadline(headline, config, watchProfile = getEffectiveWatchProfile(config)) {
...
function buildRationale(headline, axes, sc) {
  ...
  else if (headline.urgency === 'critical') parts.push(getDomainPack()?.id === 'cyber' ? assessUrgency(headline).reason : r.critical);
