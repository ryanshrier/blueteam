  ...
  try {
    ...
    const lines = headlines.map((h, i) => {
      const comps = h.scoreComponents
        ? Object.entries(h.scoreComponents).map(([k, v]) => `${k}=${v}`).join(' ')
        : '';
      return `${String(i + 1).padStart(3)}. [${h.score || 0}] H${h.horizon} [${h.source}] ${h.title}` +
        `${h.corroboration > 1 ? ` (×${h.corroboration})` : ''}` +
        `${h.alertMatched ? ' *ALERT*' : ''}` +
        `${h.originalHorizon ? ` (promoted from H${h.originalHorizon})` : ''}\n     ${comps}`;
    });
