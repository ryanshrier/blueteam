...
import { buildGroundingManifest, CISA_KEV_CATALOG_URL } from './grounding.js';
...
The \`[Horizon N]\` token is machine metadata only. Never mention horizon numbers elsewhere in reader-facing prose; communicate priority as ${priorityLanguage} through the action and decision horizon. Keep the title to roughly 6–10 words. Move technical identifiers, severity scores, dates, and version strings into "What happened" unless one is essential to distinguish the issue.
...
**Decision window:** A categorical decision horizon, using exactly one of: "Current shift" · "72 hours" · "7 days" · "30 days" · "quarter". This field is neither a source deadline nor an action target: do not put a calendar date, clock time, timezone, or "close of business" here.
...
• Cite sources at the end of each "What happened." When the input provides a URL for the source, render the citation as a markdown link: [Source Name, Date](URL) — so the reader can verify in one click. When a source is explicitly marked URL unavailable, cite it only as plain [Source Name, Date]; never guess, reconstruct, or search for a plausible URL. Cite only sources actually present in the input.
• ${sourceFreshness}
• ${certaintyLanguage}
 ...
 */
export function buildUserPrompt({ headlines, continuityContext, groundTruth, config, groundingManifest = null }) {
  ...
  if (headlines.length > 0) {
    ...
    for (const [horizon, items] of Object.entries(byHorizon)) {
      ...
      newsContext += `\nTier ${horizon} — ${name}:\n` + items.map(({ headline: h, grounding: sourceGrounding }) => {
        ...
        if (sourceGrounding?.url) {
          e += `\n  URL: <source>${encodeSourceMetadata(sourceGrounding.url)}</source>`;
        } else {
