...
• Cite sources at the end of each "What happened." When the input provides a URL for the source, render the citation as a markdown link: [Source Name, Date](URL) — so the reader can verify in one click. When a source is explicitly marked URL unavailable, cite it only as plain [Source Name, Date]; never guess, reconstruct, or search for a plausible URL. Use the source's provided Published date exactly; a citation date must never be later than the briefing's edition date. Cite only sources actually present in the input.
• ${sourceFreshness}
• ${certaintyLanguage}
