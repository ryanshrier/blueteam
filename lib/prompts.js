...
Use the retained substantive passage, not plausible details suggested by a headline. Every Key Judgment needs at least one substantive cited passage. Title-only, limited, or contaminated inputs may become attributed Developing/Watchlist leads with an explicit evidence gap; they do not justify a detailed Key Judgment. Sparse evidence warrants fewer judgments and an explicit coverage limit. Structured lookup fields support only their explicit details, and catalog records support only their captured facts. Neither establishes campaign scale, timing, victim impact, or comparative rankings. Retrospective numeric intervals (such as hours from disclosure to exploitation) and rankings (such as most repeatedly breached) must appear in a cited passage; do not infer them from publication timestamps or incident repetition. Check that every closed enumeration agrees with its stated count.

**${brief.fields.impactLabel}:** ${brief.fields.impactInstruction}

**Relevance:** When reporting mentions a declared technology, sector, region or watch topic, explain that match and the applicability check still needed. A declared interest is not a named vendor confirmed in the local stack. Say exposure is unknown unless explicit organizational evidence establishes otherwise. If no declared relevance is supported, omit this line rather than forcing it.
...
${WATCHLIST_MIN_ITEMS}–${WATCHLIST_MAX_ITEMS} bullets. Each one observable and binary: it happens or it does not. ${brief.exemplars.watchlist}
