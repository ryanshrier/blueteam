...
import { getStorageHealth, resetStorageHealth } from './storage-health.js';
...
CREATE TABLE IF NOT EXISTS scheduled_brief_jobs (
...
CREATE TABLE IF NOT EXISTS brief_index_state (
...
const MIGRATIONS = [
  ...
  // v5 → v6: track the content represented by each FTS row so startup can
  // reconcile edited and deleted Markdown files, and add a bounded persistent
  // cache for expensive non-feed egress (article extraction/search discovery).
  ...
  // v6 -> v7: map each unattended daily job to its immutable edition and
  // published archive. The filesystem archive is written first; this marker is
  // then inserted synchronously. On a crash between those operations the
  // deterministic -00 archive lets the route reconstruct this row without
  ...
  // v7 -> v8: bounded source observations and immutable collected passages.
  // Kept separately from lossy headline grouping and its short trend archive.
  (database) => database.exec(`
    ...
    CREATE INDEX idx_evidence_sources_observed ON evidence_sources(last_observed_at);
    CREATE TABLE evidence_aliases (
      alias_key TEXT PRIMARY KEY,
      source_id TEXT NOT NULL REFERENCES evidence_sources(source_id) ON DELETE CASCADE
    );
    CREATE INDEX idx_evidence_aliases_source ON evidence_aliases(source_id);
    CREATE TABLE evidence_revisions (
      ...
      passage_truncated INTEGER NOT NULL DEFAULT 0,
      published_at TEXT,
    ...
    CREATE INDEX idx_evidence_revisions_source ON evidence_revisions(source_id, revision_number DESC);
  `),
  // v9: immutable first-observed ranking evidence. Do not backfill old rows
  // from today's labels: that would manufacture hindsight-free history.
  database => database.exec('ALTER TABLE headline_archive ADD COLUMN first_snapshot_json TEXT'),
];
...
export function getDatabaseHealth() {
  let readable = false;
  try { readable = Boolean(db?.prepare('SELECT 1 AS ready').get()?.ready); } catch { /* safe status only */ }
  const persistence = getStorageHealth();
  return { status: readable && persistence.status === 'ok' ? 'ok' : 'error', readable, persistence };
}
...
export function getScheduledBriefJob(jobKey) {
  return db.prepare('SELECT * FROM scheduled_brief_jobs WHERE job_key = ?').get(jobKey) || null;
}

export function completeScheduledBriefJob({
  jobKey,
  editionDate,
  timezone,
  filename,
  ...
  const values = [jobKey, editionDate, timezone, filename, completedAt];
  if (values.some(value => typeof value !== 'string' || value.length === 0)) {
    throw new TypeError('scheduled brief completion fields must be non-empty strings');
  }
  const complete = db.transaction(() => {
    const existing = getScheduledBriefJob(jobKey);
    if (existing) {
      if (
        existing.edition_date !== editionDate
        || existing.timezone !== timezone
        || existing.filename !== filename
      ) {
        throw new Error(`Scheduled brief job ${jobKey} is already bound to a different publication`);
      }
      return existing;
    }
    db.prepare(`
      INSERT INTO scheduled_brief_jobs
        (job_key, edition_date, timezone, filename, completed_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(jobKey, editionDate, timezone, filename, completedAt);
    return getScheduledBriefJob(jobKey);
  });
  return complete();
}
...
export function indexBrief(filename, content) {
  ...
  const replace = db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO brief_index_state (filename, content_hash, indexed_at)
      VALUES (?, ?, datetime('now'))
      ON CONFLICT(filename) DO UPDATE SET
        content_hash = excluded.content_hash,
        indexed_at = excluded.indexed_at
    `).run(filename, createHash('sha256').update(content).digest('hex'));
  });
  replace();
}

export function searchBriefs(query, limit = 20, { sort = 'relevance' } = {}) {
  return db.prepare(`
    SELECT brief_search.filename, snippet(brief_search, 1, '<mark>', '</mark>', '...', 40) as snippet,
      brief_meta.generated_at as generatedAt, brief_meta.word_count as wordCount
    FROM brief_search
    LEFT JOIN brief_meta ON brief_meta.filename = brief_search.filename
    WHERE content MATCH ?
    ORDER BY ${sort === 'newest' ? 'COALESCE(brief_meta.generated_at, brief_search.filename) DESC, brief_search.filename DESC' : 'rank'}
    LIMIT ?
  `).all(query, limit);
}
...
export function backfillBriefSearch(briefsDir) {
  ...
  try {
    ...
    const state = new Map(
      db.prepare('SELECT filename, content_hash FROM brief_index_state').all()
        .map(row => [row.filename, row.content_hash])
    );
    const readable = [];
    for (const filename of files) {
      try {
        const content = readFileSync(join(briefsDir, filename), 'utf-8');
        const hash = createHash('sha256').update(content).digest('hex');
        readable.push({ filename, content, hash });
      } catch {
        // Preserve the prior searchable row for a transiently unreadable file.
      }
    }

    const replaceSearch = db.prepare('DELETE FROM brief_search WHERE filename = ?');
    const insertSearch = db.prepare('INSERT INTO brief_search (filename, content) VALUES (?, ?)');
    const upsertState = db.prepare(`
      ...
      ON CONFLICT(filename) DO UPDATE SET
        ...
        indexed_at = excluded.indexed_at
    `);
    const deleteState = db.prepare('DELETE FROM brief_index_state WHERE filename = ?');
    const indexedFilenames = new Set(
      db.prepare('SELECT DISTINCT filename FROM brief_search').all().map(row => row.filename)
    );
    const tx = db.transaction(() => {
      for (const entry of readable) {
        if (state.get(entry.filename) === entry.hash && indexedFilenames.has(entry.filename)) continue;
        replaceSearch.run(entry.filename);
        insertSearch.run(entry.filename, entry.content);
        upsertState.run(entry.filename, entry.hash);
        indexed++;
      }
      for (const filename of indexedFilenames) {
        if (onDisk.has(filename)) continue;
        replaceSearch.run(filename);
  ...
  if (indexed > 0 || removed > 0) {
    log.info('db', `FTS5 reconciliation: indexed ${indexed}, removed ${removed}`);
  }
...
// ── KEV cache ──
// Persistent cache for expensive non-feed egress.
export function getExternalCache(cacheKey, { allowExpired = false } = {}) {
  const row = db.prepare(`
    SELECT body, etag, last_modified, fetched_at, expires_at
    FROM external_cache
    WHERE cache_key = ?
  `).get(cacheKey);
  if (!row) return null;
...
export function setExternalCache(cacheKey, body, {
  ttlMs,
  etag = null,
  lastModified = null,
} = {}) {
  if (!Number.isFinite(ttlMs) || ttlMs <= 0) {
    throw new Error('external cache ttlMs must be positive');
  }
...
export function pruneExternalCache(retainExpiredDays = 7) {
  const boundedDays = Math.max(0, Math.min(90, Number(retainExpiredDays) || 0));
  return db.prepare(
    "DELETE FROM external_cache WHERE expires_at < datetime('now', '-' || ? || ' days')"
  ).run(boundedDays).changes;
}
...
/** Retain each matched CVE's identity and deadline, including entries without a due date. */
export function getKEVRecords(cveIds) {
  const ids = [...new Set((cveIds || []).filter(id => /^CVE-\d{4}-\d{4,7}$/.test(id)))].slice(0, 2000);
  if (!ids.length) return {};
  const today = new Date().toISOString().slice(0, 10);
  const rows = db.prepare(`SELECT cve_id, vendor, product, vulnerability_name, date_added, due_date
    FROM kev_cache WHERE cve_id IN (${ids.map(() => '?').join(',')})`).all(...ids);
  return Object.fromEntries(rows.map(row => [row.cve_id, {
    cve: row.cve_id, vendor: row.vendor, product: row.product, name: row.vulnerability_name,
    dateAdded: row.date_added, dueDate: row.due_date, overdue: Boolean(row.due_date && row.due_date < today),
  }]));
}
...
export function archiveHeadlines(headlines, context = {}) {
  const upsert = db.prepare(`
    INSERT INTO headline_archive
      (title, title_key, source, link, horizon, score, urgency, is_kev, kev_cve, corroboration, publishers_json, published_at, first_snapshot_json)
    VALUES (@title, @title_key, @source, @link, @horizon, @score, @urgency, @is_kev, @kev_cve, @corroboration, @publishers_json, @published_at, @first_snapshot_json)
  ...
  const tx = db.transaction(() => {
    for (const [index, h] of headlines.entries()) {
      ...
      upsert.run({
        ...
        first_snapshot_json: JSON.stringify({
          version: 1, observedAt: context.observedAt || new Date().toISOString(),
          rank: index + 1, candidateCount: headlines.length,
          title: h.title, score: h.score || 0, horizon: h.horizon,
          urgency: h.urgency || 'routine', isKEV: Boolean(h.isKEV), kevCVE: h.kevCVE || null,
          weight: h.weight ?? 1, cvssScore: h.cvssScore ?? null, epss: h.epss ?? null,
          scoreComponents: h.scoreComponents || null,
          configuration: context.scoringConfiguration || null,
        }),
      });
    }
  });
