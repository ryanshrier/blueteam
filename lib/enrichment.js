...
import {
  getKEVSet, bulkInsertKEV, getKEVAge, getKEVDatesAdded, getKEVRecords,
...
import { retainedText, headlineCves } from './intelligence-context.js';
  ...
  if (_matchersCache.id !== pack.id) {
    _matchersCache = {
      ...
      actors: (pack.entities?.actors || []).map(actor => ({
        name: actor.name,
        region: actor.region,
        family: PATTERN_ACTOR_SHAPE.test(actor.name),
...
export function matchActors(title, body = '') {
  const out = [];
  const actors = entityMatchers().actors;
  for (const m of actors.filter(actor => !actor.family)) {
  ...
  // Interim designators identify distinct clusters, not one umbrella actor.
  // Emit the observed text and let known names/aliases own their designators
  // (UNC3944 is Scattered Spider, not a second unattributed actor).
  const knownDesignator = value => actors.some(actor => !actor.family
    && actor.patterns.some(pattern => pattern.exec(value)?.[0].length === value.length));
  const seen = new Set();
  for (const m of actors.filter(actor => actor.family)) {
    for (const [text, basis] of [[title || '', 'title'], [body || '', 'mention']]) {
      for (const pattern of m.patterns) {
        for (const match of text.matchAll(new RegExp(pattern.source, 'gi'))) {
          ...
          if (seen.has(identity) || isNegatedAt(text, match.index) || knownDesignator(match[0])) continue;
          seen.add(identity);
          out.push({ name: match[0], region: m.region, basis });
        }
      }
    }
  }
...
const ARTICLE_CACHE_TTL_MS = 12 * 60 * 60 * 1000;
const ARTICLE_STALE_MAX_MS = 7 * 24 * 60 * 60 * 1000;

function externalCacheKey(namespace, value) {
  return `${namespace}:${createHash('sha256').update(String(value)).digest('hex')}`;
}

function cachedExternal(key) {
  try { return getExternalCache(key, { allowExpired: true }); } catch { return null; }
}

function persistExternal(key, body, options) {
  try { setExternalCache(key, body, options); } catch { /* database may not be ready in an embedder */ }
}

function loggableUrl(value) {
  try {
...
function acceptStaleArticle(cached) {
  const fetchedAt = Date.parse(cached?.fetched_at || '');
  return cached?.body
    && !Number.isNaN(fetchedAt)
    && (Date.now() - fetchedAt) <= ARTICLE_STALE_MAX_MS;
}

export async function extractArticleBody(url, timeoutMs = 3000, outcome = {}) {
  const cacheKey = externalCacheKey('article', url);
  const cached = cachedExternal(cacheKey);
  if (cached && !cached.expired) return cached.body;

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const headers = { 'User-Agent': outboundUserAgent(), 'Accept': 'text/html' };
    ...
    if (res.status === 304 && cached?.body) {
      ...
      persistExternal(cacheKey, cached.body, {
        ttlMs: ARTICLE_CACHE_TTL_MS,
        etag: cached.etag,
    ...
    if (!res.ok) {
      await discardResponseBody(res);
      outcome.failed = true;
      return acceptStaleArticle(cached) ? cached.body : null;
    }
    const html = await readCapped(res, 2_000_000);
    let body = domExtract(html);
    if (!body || body.length < 50) { outcome.failed = true; return acceptStaleArticle(cached) ? cached.body : null; }
    body = body.replace(/\s+/g, ' ').trim().slice(0, 2000);
    persistExternal(cacheKey, body, {
      ttlMs: ARTICLE_CACHE_TTL_MS,
    ...
    outcome.failed = true;
    log.warn('enrichment', `Article extraction failed for ${loggableUrl(url)}: ${err.message}`);
    return acceptStaleArticle(cached) ? cached.body : null;
  } finally {
...
async function performKEVRefresh() {
  try {
    const age = getKEVAge();
    if (age < KEV_REFRESH_HOURS && kevSet) { kevRefreshFailed = false; return kevSet; }
...
export async function enrichKEV(headlines) {
  ...
  if (!kev || kev.size === 0) {
    if (kevRefreshFailed) throw new Error('KEV catalog unavailable');
    return;
  }

  let hits = 0;
  let records = {};
  try { records = getKEVRecords(headlines.flatMap(headlineCves)); } catch { /* catalog set remains useful */ }
  for (const h of headlines) {
    const matched = headlineCves(h).filter(cve => kev.has(cve));
    h.kevRecords = matched.map(cve => records[cve] || { cve });
    h.isKEV = matched.length > 0;
    h.kevCVE = matched[0] || null;
    if (h.isKEV) {
      hits++;
      h.kevDateAdded = h.kevRecords.map(record => record.dateAdded).filter(Boolean).sort().at(-1) || null;
    }
  }
  if (hits > 0) {
    ...
    try {
      ...
      for (const h of matched) if (!h.kevDateAdded && dates[h.kevCVE]) h.kevDateAdded = dates[h.kevCVE];
    } catch { /* non-critical */ }
    log.info('enrichment', `KEV matches: ${hits} headlines reference known-exploited CVEs`);
  }
  if (kevRefreshFailed) throw new Error('KEV refresh failed; retained catalog used');
}
...
const CVE_UNAVAILABLE = Symbol('cve-unavailable');
...
export async function enrichCVEs(headlines, maxLookups = 8) {
    ...
    for (const cve of cves) {
      try {
        ...
        if (data === undefined) {
          ...
          if (throttled || unavailable || attempts >= maxLookups) continue;
          attempts++;
  ...
  if (unavailable) throw new Error('NVD unavailable; some CVE details could not be verified');
}
...
async function fetchCVEData(cveId) {
  try {
    ...
    if (res.status === 403 || res.status === 429) {
      await discardResponseBody(res);
      return CVE_THROTTLED;
    }
    if (!res.ok) {
      await discardResponseBody(res);
      return CVE_UNAVAILABLE;
    }

    const data = JSON.parse(await readCapped(res, 4_000_000));
    if (!Array.isArray(data?.vulnerabilities)) return CVE_UNAVAILABLE;
    const vuln = data?.vulnerabilities?.[0]?.cve;
    if (!vuln) return null;
    if (vuln.id && vuln.id.toUpperCase() !== cveId) return CVE_UNAVAILABLE;
...
export async function enrichEPSS(headlines, maxLookups = 20) {
  ...
  if (uncached.length > 0) {
    try {
      ...
      if (!Array.isArray(body?.data)) throw new Error('Invalid EPSS response');
      const now = Date.now();
      for (const row of body?.data || []) {
        const score = parseFloat(row.epss);
        if (row.cve && Number.isFinite(score) && score >= 0 && score <= 1) epssCache.set(row.cve.toUpperCase(), { score, cachedAt: now });
      }
    } catch (err) {
      lookupFailed = true;
      log.warn('enrichment', `EPSS lookup failed: ${err.message}`);
      // Apply cached estimates below before reporting the partial failure.
    }
  }
  ...
  if (lookupFailed) throw new Error('FIRST EPSS unavailable; retained estimates applied where available');
}
...
export async function enrichArticleBodies(headlines, maxExtractions = 10) {
  ...
  const results = await Promise.allSettled(
    candidates.map(async (h) => {
      const outcome = {};
      const body = await extractArticleBody(h.link, 3000, outcome);
      h.articleStale = Boolean(outcome.failed && body);
  ...
  if (extracted < candidates.length) throw new Error(`Article extraction incomplete (${extracted}/${candidates.length} current)`);
}
...
export function tagEntities(headlines) {
  for (const h of headlines) {
    ...
    const actors = matchActors(h.title || '', retainedText(h));
    if (actors.length > 0) h.actors = actors;

    const vendors = matchVendors(retainedText(h));
