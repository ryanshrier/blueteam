...
export function listBriefEditions(historyDir, { getMeta = () => null, limit = Infinity } = {}) {
  if (limit <= 0) return [];
  return readdirSync(historyDir).filter(validBriefFilename).map(filename => {
    ...
    if (!Number.isFinite(Date.parse(generatedAt || ''))) {
      try { generatedAt = readGenerationManifest(historyDir, filename)?.generatedAt; } catch { /* missing/corrupt receipt uses legacy file time */ }
    }
    const generatedAtMs = Number.isFinite(Date.parse(generatedAt || ''))
      ? Date.parse(generatedAt)
      : statSync(join(historyDir, filename)).mtimeMs;
    return { filename, date: briefDateFromFilename(filename), generatedAt: new Date(generatedAtMs).toISOString(), generatedAtMs, meta };
...
export function loadRecentBriefs(historyDir, depth = 5, options = {}) {
  ...
  try {
    const files = listBriefEditions(historyDir, options).map(edition => edition.filename);
