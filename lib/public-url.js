...
export function requestBaseUrl(req, configuredBaseUrl = null) {
  ...
  // A configured subnet/hop policy is not blanket trust: Express compiles it
  // into a predicate, and only the directly connected peer can supply these
  // headers. Mirror Express's protocol/hostname trust decision.
  const trust = req?.app?.get('trust proxy fn');
  const peer = req?.socket?.remoteAddress;
  const trustProxy = typeof trust === 'function' && typeof peer === 'string' && trust(peer, 0);
