    ...
    dispatch(opts, handler) {
      ...
      try {
        ...
        req = mod.request({
          ...
          const rawHeaders = [];
          for (const [k, v] of Object.entries(res.headers)) rawHeaders.push(k, Array.isArray(v) ? v.join(', ') : v);
          handler.onHeaders(res.statusCode, rawHeaders, () => {}, res.statusMessage);
          res.on('data', (chunk) => handler.onData(chunk));
          res.on('end', () => handler.onComplete([]));
          res.on('error', (err) => handler.onError(err));
        });
        ...
        try { handler.onConnect?.(() => req.destroy()); } catch { /* handler may not implement */ }
        req.on('error', (err) => { try { handler.onError(err); } catch { /* already settled */ } });
