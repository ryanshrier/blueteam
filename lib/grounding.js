...
export function visibleHeadlineEvidence(headline = {}, { includeEnrichment = true } = {}) {
  return [
    ...
    headline.articleBody && !headline.articleStale
...
export function buildGroundingManifest({ headlines = [], extraSourceText = '', kevSet = null } = {}) {
  ...
  const sources = sourceList.map((headline, index) => {
    ...
    const groupSources = selected.map((member, memberIndex) => {
      ...
      const evidenceText = visibleHeadlineEvidence(passage, { includeEnrichment: false });
      ...
      const source = Object.freeze({
        ...
        passage: memberIndex === 0 && member.articleBody && !member.articleStale
          ? asText(member.articleBody).slice(0, 800) : asText(passage.description),
        sourceRevisions: (memberIndex === 0 ? primaryOriginal?.evidence || (!originals.length ? member.evidence : []) : member.evidence) || [],
      });
  ...
  for (const record of nvdRecords.values()) members.push(Object.freeze(record));
