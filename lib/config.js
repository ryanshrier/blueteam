...
import { WATCH_PROFILE_LIMITS, sanitizeWatchProfile } from './watch-profile.js';
...
const AnalysisSettingsSchema = z.object({
  ...
  generationTimeoutSec: z.number().int()
    ...
    .default(180),
  // Keep more of the shared output budget available for the cited document.
  // 'off' explicitly disables thinking; operators can still select higher effort.
  thinkingEffort: z.enum(['off', 'low', 'medium', 'high']).default('low'),
...
const ConfigSchema = z.object({
  ...
  watchProfile: z.object({
    schemaVersion: z.literal(1).optional(),
    ...Object.fromEntries(Object.entries(WATCH_PROFILE_LIMITS).map(([key, limits]) => [
      key, z.array(z.string().trim().max(limits.length)).max(limits.count).optional(),
    ])),
    teamProfile: z.string().trim().max(512).optional(),
    preferredHorizons: z.array(z.union([z.literal(1), z.literal(2), z.literal(3)])).max(3).optional(),
  }).strict().transform(sanitizeWatchProfile).optional(),
