...
const implementation = Object.fromEntries([
  'prompts', 'grounding', 'validation', 'scoring',
...
function snapshotHeadline(h, index) {
  const articleExcerpt = typeof h.articleBody === 'string' && !h.articleStale ? h.articleBody.slice(0, 800) : '';
  const excerpt = articleExcerpt || text(h.description, 32768);
  return {
    ...
    passage: { kind: articleExcerpt ? 'article-opening' : excerpt ? 'feed-excerpt' : 'title-only', text: excerpt || text(h.title) },
...
export function recordProviderAttempt(manifest, params) {
  ...
  const entry = {
    ...
    thinkingEffort: text(params.output_config?.effort, 32),
  };
...
export function recordValidation(manifest, draft, result) {
  ...
  manifest.validation.push({ draftSha256: sha256(draft), valid: result.valid === true, warnings: list(result.warnings, item => text(item, 4096), 100), issues: list(result.issues, issue => ({ code: text(issue.code, 128), severity: text(issue.severity, 32), message: text(issue.message, 4096) }), 100) });
}
