...
import {
  ...
  getKEVDeadlines, countKEVOverdue, countKEVDueSoon, getKEVDueDates, getKEVRecords, titleKey,
} from './db.js';
import { getFeedHealth } from './feeds.js';
import { summarizeEvidence } from './evidence-freshness.js';
import { headlineCves, editorialContext, enrichmentStatus, normalizeFeedTimestamp, readableExcerpt } from './intelligence-context.js';
...
export function buildConvergenceClusters(headlines, limit = 5) {
  ...
  const addToCluster = (key, label, type, headline) => {
    ...
    if (Array.isArray(headline.publishers) && headline.publishers.length) {
      ...
      let identity = String(headline.source).trim().toLowerCase();
      try { identity = new URL(headline.link).hostname.toLowerCase().replace(/^www\./, ''); } catch { /* legacy source label */ }
      cluster.publishers.add(identity);
    }
  ...
  for (const h of headlines) {
    ...
    if (trackCves) {
      const cves = new Set(headlineCves(h));
      for (const cve of cves) addToCluster(cve.toUpperCase(), cve.toUpperCase(), 'cve', h);
    }
  }
  ...
  const summaries = [...clusters.values()]
    .map(c => ({
      ...
      label: c.label,
      type: c.type,
      scope: c.type === 'cve' ? 'vulnerability' : 'topic',
      meaning: 'Related reporting; source count does not establish independent confirmation or a shared attack mechanism.',
      count: c.count,
  ...
  // Suppress a broad topic only when it covers exactly the same selected stories
  // as a more precise CVE group. Additional topic developments remain visible.
  const membership = cluster => new Set(headlines.filter(h => cluster.type === 'cve' ? headlineCves(h).includes(cluster.label)
    : cluster.type === 'vendor' ? (h.vendors || []).includes(cluster.label) : (h.actors || []).some(a => a.name === cluster.label)).map(h => h.link || h.title));
  return summaries.filter(cluster => cluster.type === 'cve' || !summaries.some(other => {
    if (other.type !== 'cve') return false;
    const left = membership(cluster), right = membership(other);
...
function compactHeadline(h, kevDueDates = {}, kevById = {}, failures = []) {
  const kevCVE = h.kevCVE || h.kev_cve || null;
  const due = (kevCVE && kevDueDates[kevCVE]) || null;
  const kevRecords = [...new Set([...headlineCves(h), kevCVE].filter(Boolean))].flatMap(cve => kevById[cve] ? [kevById[cve]] : []);
  return {
    title: h.title,
    editorialContext: editorialContext(h, kevRecords),
    kevRecords,
    enrichmentStatus: enrichmentStatus(h, failures),
    source: h.source,
    link: h.link || null,
    horizon: h.horizon || 2,
    ...
    date: normalizeFeedTimestamp(h.date || h.published_at) || null,
    dateUnknown: !normalizeFeedTimestamp(h.date || h.published_at),
    retrievedAt: h.retrievedAt || null,
    collectionStale: Boolean(h.collectionStale),
    description: h.description || '',
    displayExcerpt: readableExcerpt(h.passage || h.description || '', 480),
    descriptionTruncated: Boolean(h.passageTruncated || (h.passage || h.description || '').length > 480),
...
export function buildLandscape(run, latestBrief = null, { runAgeMs = null } = {}) {
  ...
  if (panels.has('kev')) try {
    ...
    const ids = headlines.flatMap(h => [...headlineCves(h), h.kevCVE || h.kev_cve].filter(Boolean));
    kevDueDates = getKEVDueDates(ids);
  ...
  for (const n of [1, 2, 3]) {
    ...
    const top = items.slice(0, 1).map(h => compactHeadline(h, kevDueDates, kevById, run?.stats?.enrichmentFailures))[0] || null;
    horizons[n] = {
      count: items.length,
  ...
  // but is quiet (empty) is reachable. Stale fallback means the origin failed;
  // local cache availability must not conceal that outage in the health count.
  ...
  const evidence = summarizeEvidence(headlines);
  const collection = run?.stats?.collection || null;
  const inadequateCoverage = collection?.configuredSources > 0 && collection.freshSources / collection.configuredSources < 0.5;

  return {
    generatedAt: evidence.observedAt,
    refreshedAt: run?.generatedAt || null,
    evidence,
    collection,
    stale: !run || ageMs > pipelineStaleAfterMs(refreshMinutes) || evidence.freshHeadlines < 5 || inadequateCoverage,
    horizons,
    signals: headlines.slice(0, 14).map(h => compactHeadline(h, kevDueDates, kevById, run?.stats?.enrichmentFailures)),
