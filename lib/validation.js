...
import {
  ...
  section, splitEntries, field, hasField, parseBluf, stripMd,
 ...
 * all flagged. routes/brief.js treats the factual grounding subset as
 * publication-blocking after one corrective retry.
 *
 * `source` accepts:
...
const TRUST_CRITICAL_RE = /^(?:Ungrounded CVE\(s\)|CVE\(s\) labeled KEV but not|CVE\(s\) described as pending\/not in KEV|KEV catalog unavailable|Unverifiable source link\(s\)|Raw HTML anchor\(s\))/i;
...
/** True for a factual grounding warning that cannot be published unresolved. */
