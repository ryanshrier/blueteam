...
const alertWork = new Set(); // delivery may outlive the fresh-run response
let timer = null;
...
function persistLatestRun(run) {
  try {
    ...
    recordStorageOutcome('latest-run');
  } catch (err) {
    recordStorageOutcome('latest-run', err);
...
export class EvidenceUnavailableError extends Error {
  constructor(message, details = {}) {
    ...
    this.code = 'E_EVIDENCE';
    this.details = details;
  }
}

/**
 * Refuse to hand a paid generation an empty, undersized, or stale evidence set.
 * The Wall may honestly retain an old last-good snapshot during an outage; a
 * new Briefing must not present that retained snapshot as current evidence.
 */
export function requireAdequateEvidence(run, {
  minHeadlines = 5,
  maxAgeMs = 30 * 60_000,
  minFreshSourceRatio = 0.5,
} = {}) {
  const headlines = Array.isArray(run?.headlines) ? run.headlines : [];
  const ageMs = Number.isFinite(run?.generatedAtMs)
    ? Math.max(0, Date.now() - run.generatedAtMs)
    : Infinity;
  if (headlines.length < minHeadlines) {
    throw new EvidenceUnavailableError(
      `Briefing generation needs at least ${minHeadlines} current headlines; only ${headlines.length} are available.`,
      { headlines: headlines.length, minHeadlines, ageMs: Number.isFinite(ageMs) ? ageMs : null },
    );
  }
  if (ageMs > maxAgeMs) {
    throw new EvidenceUnavailableError(
      `Briefing evidence is stale (${Math.ceil(ageMs / 60_000)} minutes old). Refresh the landscape and try again.`,
      { headlines: headlines.length, minHeadlines, ageMs, maxAgeMs },
    );
  }
  const evidence = summarizeEvidence(headlines, { maxAgeMs });
  const collection = run?.stats?.collection;
  if (evidence.freshHeadlines < minHeadlines || (collection?.configuredSources > 0 &&
      collection.freshSources / collection.configuredSources < minFreshSourceRatio)) {
    throw new EvidenceUnavailableError(
      'Current source coverage is insufficient for a new briefing. Retained or unverified cache content remains available on the Wire.',
      { ...evidence, minHeadlines, maxAgeMs, collection: collection || null },
    );
  }
  return run;
}
...
export async function refreshNow(reason = 'manual') {
  ...
  refreshInFlight = (async () => {
    ...
    latestRun = {
      ...
      evidence: summarizeEvidence(result.headlines),
    };

    try {
      archiveHeadlines(result.headlines, { observedAt: latestRun.generatedAt, scoringConfiguration: result.scoringConfiguration });
      pruneHeadlineArchive(config.analysisSettings?.headlineArchiveDays ?? 14);
      pruneFeedHealth(7);
      recordStorageOutcome('archive');
    } catch (err) {
      recordStorageOutcome('archive', err);
      log.warn('refresher', `Archive write failed (non-blocking): ${err.message}`);
    }
    try {
      pruneExternalCache(7);
    } catch (err) {
      log.warn('refresher', `External cache prune failed (non-blocking): ${err.message}`);
    }
    ...
    const delivery = Promise.resolve().then(() => dispatchAlerts(result.headlines, config))
      .catch(err => log.warn('refresher', `Alert dispatch failed (non-blocking): ${err.message}`));
    alertWork.add(delivery);
    delivery.then(() => alertWork.delete(delivery), () => alertWork.delete(delivery));

    return latestRun;
  })().finally(() => {
    // Attach cleanup after promise assignment, including synchronous config or
    // pipeline failures that occur before the first await.
 ...
 */
export async function getFreshRun(maxAgeMs = 5 * 60_000, evidenceRequirements = null) {
  const run = latestRun && getRunAgeMs() <= maxAgeMs
    ? latestRun
    : await refreshNow('stale-on-demand');
  return evidenceRequirements ? requireAdequateEvidence(run, evidenceRequirements) : run;
}
...
/** Call after stopping schedules and draining accepted HTTP/Briefing work. */
export async function waitForRefreshIdle() {
  while (refreshInFlight || alertWork.size) {
    await Promise.allSettled([refreshInFlight, ...alertWork].filter(Boolean));
  }
}
